...
$day_after_tomorrow_date_time=date("Y-m-d H:i:s", $day_after_tomorrow_date_time);
 ...
 if($record_type=='1') {
        $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,AffectedBoundaries FROM Alerts where topic_id in (".$topicIDs.") AND  Sent >='".$todaydate."' AND Sent <='".$todaydateEnd."' AND Expires < '".$tomorrow_dateEnd_time."' ORDER BY `Sent` DESC";
    } else {
