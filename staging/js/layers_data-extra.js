         ...
         { 
           ...
           if(view.checked==true)
           {
           	...
           	if ((jQuery.inArray(view.value,total_loaded_layer))!=-1)
           	{
					...
					if(view.value!='1' && public_checked_options.length != 1){           	
						$('.assets_report').prop('disabled', false);
						$('.report_disabled').hide();
					}
					setting[10].public_layer_option=public_checked_options;
    	...
    	onCheckAll: function()
    	{
			...
			$('#showClientDataDropDownDiv input:checked').each(function()
			{
				...
				if(this.value!=='on') {
					if ((jQuery.inArray(this.value,current_selected_layer))==-1)
					{
						if(this.value!=='12' && this.value!=='19' && this.value!=='38' && this.value!=='39' && this.value!=='45' && this.value!=='46' && this.value!=='47' && this.value!=='52' && this.value!=='53') {
...
onUncheckAll:function()
{
         ...
         onClick:function(view)
         {
            ...
            if(view.checked==true)
            {
            	...
            	client_loaded_count=0;
            	if(view.value!=='12' && this.value!=='19' && view.value!=='38' && view.value!=='39' && view.value!=='45' && view.value!=='46' && view.value!=='47' && view.value!=='52' && view.value!=='53') {
            		$('#site_list_option').show();
            		if($("#show_sites_layers option[value='"+this.value+"']").length > 0) {
            		} else {
            			$('#show_sites_layers')
...
RT_data = [];
function load_RT_data(){
	if(RT_data.length > 0){
		hide_show_RT_data(true,function(){
			hide_loader("public")
		});
	} else {
		show_loader("public");
		$.getJSON('json_data/RT.json',"", function(json, textStatus) {
			$temp_arr = [];
			for(var i= 0; i<json.length; i++){
				var flightPlanCoordinates = [];
				for(var j=0; j<json[i].length; j++){
					var data = [];
					latlongpair = json[i][j].split(",");
					data.lng = parseFloat(latlongpair[0]);
					data.lat = parseFloat(latlongpair[1]);
...
function hide_show_RT_data(flag,callback){
	if(RT_data.length > 0){
		count = 0;
		show_loader("public");
		setTimeout(function(){
			for(var i=0; i<RT_data.length; i++){
				RT_data[i].setVisible(flag);
			}
			callback();//hide_loader("public");
		},10);
	}
}
...
function showPublicDataDropdown()
{
	if($('#public_data_layer').attr("checked"))
	{
		...
		if(layer_id==1) {
			...
			RT_VIS.setMap(null);
			RT_WA.setMap(null);*/
			hide_show_RT_data(false,function(){
				hide_loader("public")
			});
			$('#loader_layer_type').hide();
		}
				...
				//client_layer_count--;
				 ...
				 if(parseInt(response.layer_id)<=10) {
					...
					global_layer_marker_counter++;
				     ...
				     //$('#show_sites').show();
				     if((parseInt(layer_values)>10 || (parseInt(layer_values)==2 && (LOGIN_USER_NAME=='telstra' || LOGIN_USER_NAME=='admin'))) && (parseInt(layer_values)!==12 && parseInt(layer_values)!==19)) {
...
if(layer_id==39) {
						  ...
						  /*if(layer_id==64) {
							...
							function show_loader(layer_type) {
								...
								if(layer_type=='client'){
									...
									$("#client_data_layer").css('display', 'none').attr("disabled",true);
								} else {
									showAjaxWait($('#public_data_layer'),'public_layer_loader');
									$("#public_data_layer").css('display', 'none').attr("disabled",true);
							...
							function hide_loader(layer_type) {
								...
								if(layer_type=='client'){
									...
									$("#client_data_layer").css('display', 'inline-block').attr("disabled",false);
								} else {
									hideAjaxWait('public_layer_loader');
									$("#public_data_layer").css('display', 'inline-block').attr("disabled",false);
								}
				...
				for(z=0;z<=client_layers.length-1;z++)
				{
					...
					layer_value=client_layers[z];
								...
								if($('#showClientDataDropDownDiv').find("input[type=checkbox]").filter('[value="'+client_layers[z]+'"]').length>0) {
									...
									/* show sites */
									if(client_layers[z]!=='12' && client_layers[z]!=='19' &&  client_layers[z]!=='38' && client_layers[z]!=='39' && client_layers[z]!=='45' && client_layers[z]!=='46' && client_layers[z]!=='47' && client_layers[z]!=='52' && client_layers[z]!=='53') {
			...
			function public_layer(public_layers) {
			 ...
			 client_layer_count=1;
			 $.each(client_checked_options,function(value)
			 {
			...
			//console.log(public_checked_options);
			  //console.log(current_selected_layer);
			  public_layer_count=1;
			  $.each(public_checked_options,function(value)
			  {
