	...
	function showsatimage(when) {
		...
		if (satinit == 0) {
			...
			map.overlayMapTypes.j[0].setOpacity(parseFloat($("#sat_module_opacity").val()));
			satinit = 1;
		} else {
			if (when == "prev") {
				map.overlayMapTypes.clear();
				...
				if (sat_looping == 0) {
					map.overlayMapTypes.j[0].setOpacity(0.00);
					map.overlayMapTypes.j[1].setOpacity(parseFloat($("#sat_module_opacity").val()));
					map.overlayMapTypes.removeAt(0);
					map.overlayMapTypes.insertAt(0, satmap_next);
					sat_looping = 1;
				} else {
					map.overlayMapTypes.j[1].setOpacity(0.00);
					map.overlayMapTypes.j[0].setOpacity(parseFloat($("#sat_module_opacity").val()));
