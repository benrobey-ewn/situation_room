...
/*foreach ($polygonresponsearray as $polygon => $polygonv)
{
    ...
    foreach ($polygonv['layers'] as $key => $val) {
       // echo "<pre>";
         ...
         //print_r($key);
         //print_r($val);
        foreach ($val['placemarker_name'] as $keyv => $record)
        {
            //echo "<pre>";
           // print_r($val['polygon_latitude']);

//die;          //  echo "<pre>";
            //foreach($record as $keyp => $placemarker) {
               // echo "<pre>";
               // print_r($placemarker);
               // die;
                $csv.= ",".$record.",".$val['polygon_latitude'][$keyv].",".$val['polygon_longitude'][$keyv]."\n"; //Append data to csv
            //}
