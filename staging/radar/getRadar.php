<?php  header("Access-Control-Allow-Origin: *");
    ...
    //print_r($data_string);
    //print_r($_POST);
    $viewportLatMin= trim(mysql_real_escape_string($_POST['viewportLatMin']));
    $viewportLatMax= trim(mysql_real_escape_string($_POST['viewportLatMax']));
    
    $viewportLonMin= trim(mysql_real_escape_string($_POST['viewportLonMin']));
    $viewportLonMax= trim(mysql_real_escape_string($_POST['viewportLonMax']));
    
    $viewportZoom= trim(mysql_real_escape_string($_POST['viewportZoom']));
    $radarType= trim(mysql_real_escape_string($_POST['radarType']));
    ...
    */
    ...
    $max_time = date('Y-m-d H:i:s', strtotime('- 1 hours', strtotime($newdate)));
    $max_time = date('Y-m-d H:i:s', strtotime('- 20 minutes', strtotime($max_time)));
    ...
    if($radarType=='512') {
        ...
        WHERE
            RC.radar_type=".$radarType." AND RC.radar_key NOT IN (56,23,19,73,72,50,66,53,69,3,40,9,63,64,68,37,2,30,31,32,6,29,39,55,22,71) AND ((RC.min_long >=".$viewportLonMax." AND max_long <= ".$viewportLonMin." AND min_lat >=".$viewportLatMax." AND max_lat <=".$viewportLatMin.") OR (
        ((".$viewportLatMax." < max_lat && ".$viewportLatMin." >max_lat || ".$viewportLatMin." >min_lat && ".$viewportLatMax." <min_lat || ".$viewportLatMax." >= min_lat && ".$viewportLatMin." <= max_lat) &&  (".$viewportLonMax." < max_long && ".$viewportLonMin." >max_long || ".$viewportLonMin." >min_long && ".$viewportLonMax." < min_long || ".$viewportLonMax." >= min_long && ".$viewportLonMin." <= max_long)) )) AND RI.radar_date_time > '".$max_time."'
        ORDER BY
