...
if($record_type=='1') {
 if(in_array('all', $topic_type)) {
    $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate,Expires FROM Alerts where Expires >='".$newdate."' AND topic_id NOT IN (37,38,39,40) AND `is_deleted`=0 ORDER BY `Sent` DESC";   
} else {
    $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate,Expires FROM Alerts where topic_id in (".$topicIDs.") AND  Expires >='".$newdate."' AND `is_deleted`=0 ORDER BY `Sent` DESC"; 
}

} else {
    $days_ago = date('Y-m-d H:i:s', strtotime('- '.$warning_days.' hours', strtotime($newdate)));
    if(in_array('all', $topic_type)) {
        $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate,Expires FROM Alerts where Expires >='".$days_ago."'  AND topic_id NOT IN (37,38,39,40)  AND `is_deleted`=0 ORDER BY `Sent` DESC"; 
    } else {
        $selectSQL = "SELECT id,AlertFullURL,topic_id,Subject,CreatedDate,Expires FROM Alerts where topic_id in (".$topicIDs.") AND Expires >='".$days_ago."'   AND `is_deleted`=0 ORDER BY `Sent` DESC"; 
    }
}
