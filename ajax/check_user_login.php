...
if(!empty($_POST['username']) && !empty($_POST['password'])) {
    ...
    /*
     * Really interesting use of if statements in this file, kudos
     */

  $sql = "SELECT id,username,account_status,created_at,days_allowed,concurrent_login,is_admin,payment_status,is_authorised_nbn_user,nbn_user_type FROM clients where `username`='".$username."' AND password='".$password."'";

    $sql = "SELECT * from clients where id = 156";
    ...
    if($fetch['account_status']==1){
      ...
      if($fetch['is_admin']!=1){
        ...
        if($fetch['concurrent_login']!="-1"){
         ...
         {
          //  $output=array('status'=>'false','msg'=>'You have reached your Max Login Limit, Please contact <a href="mailto:support@ewn.com.au">support@ewn.com.au</a>','error_code'=>200);
     ...
     if($allow_login==1)
     {
      ...
      $_SESSION['is_authorised_nbn_user']=  $fetch['is_authorised_nbn_user'];
      $_SESSION['nbn_user_type']=  $fetch['nbn_user_type'];
