<?php
 ...
 //ini_set("display_errors",1);
  ...
  if(!empty($_POST['username']) && !empty($_POST['password'])) {
    ...
    $sql = "SELECT id,username,account_status,created_at,days_allowed,concurrent_login,is_admin,payment_status FROM clients where `username`='".$username."' AND password='".$password."' AND `account_status`=1";
    $rsp = mysql_query($sql);
    if(mysql_num_rows($rsp)>0) {
        ...
        if($fetch['account_status']==1){
          ...
          $output=array('status'=>'false','msg'=>'Your account is not active, Please contact your administrator',"query"=>$calulated_date);
        }
    } else {
        //$output=array('status'=>'false', 'msg'=>'Invalid username and password');
        $output=array('status'=>'false', 'msg'=>"Unable to login. Please check you have entered your password and username correctly. If you have forgotten your password or can't login, please contact <a href='mailto:support@ewn.com.au'>support@ewn.com.au</a>");
    }
  } else {
