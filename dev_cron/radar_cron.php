<?php
    ...
    function save_image($image,$radar_id,$radar_code) {
         ...
         if($count==0)  {
            ...
            imagepng($dest, '../dev/radar_images/'.$file);
            imagedestroy($png_image);
            /* image save into folder */
           
           chmod('../dev/radar_images/'.$file, 0777);
           $min=substr($time,0,2);
           $sec=substr($time,2,2);
           $time=$min.":".$sec.":"."00";
           $date_time=$date.' '.$time;
    ...
    /* Crawler Script */
    $selectSQL = "SELECT radar_id,radar_code FROM radar_codes WHERE radar_type NOT IN ('512')";
    $result = mysql_query($selectSQL);
    ...
    $selectSQL = "SELECT radar_id,radar_code FROM radar_codes WHERE radar_type NOT IN ('512')";
    $result = mysql_query($selectSQL);
    //$radar_code=mysql_fetch_array($result);
    while($radar_code=mysql_fetch_array($result)) {
...
AND image_id IN (SELECT image_name
     ...
     ORDER BY radar_date_time DESC LIMIT 1)";
                ...
                while($del_image=mysql_fetch_array($del_result)) {
                    if (file_exists('../dev/radar_images/'.$del_image['image_name'])) {
                        unlink('../dev/radar_images/'.$del_image['image_name']);
                    } 
	...
	include '../radar_dev/includes/config.php';
    mysql_select_db("situatio_radar_dev", $radar_dev_con);


    if (!empty($query_array)) {
        foreach($query_array as $radar_query) {
            $res = mysql_query($radar_query, $radar_dev_con);
        }
    }
    
    /* delete image older than 1 hour from radar dev file  */
    $select_radar_codes = "SELECT radar_id,radar_code FROM radar_codes WHERE radar_type NOT IN ('512')";
    $result = mysql_query($select_radar_codes,$radar_dev_con);
    while($radar_code=mysql_fetch_array($result)) {
        $select_radar_images = "SELECT image_name,image_id FROM radar_images where radar_id = '".$radar_code['radar_id']."'";
        $radar_result = mysql_query($select_radar_images,$radar_dev_con);
        $total = mysql_num_rows($radar_result);
        if($total>15) {
            $del_total=$total-15; 
            $select_radar_images2 = "SELECT image_id,image_name FROM radar_images WHERE radar_id='".$radar_code['radar_id']."' ORDER BY radar_date_time ASC LIMIT ".$del_total;
            $del_result = mysql_query($select_radar_images2,$radar_dev_con);
            while($del_image=mysql_fetch_array($del_result)) {
                $del_radar_images = "DELETE FROM radar_images where image_id = '".$del_image['image_id']."'";
                $res = mysql_query($del_radar_images,$radar_dev_con);
            }
        }
    }

    mysql_close($radar_dev_con);
