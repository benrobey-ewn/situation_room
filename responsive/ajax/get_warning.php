...
/*if($record_type=='1') {
     if(in_array('all', $topic_type)) {
        $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,AffectedBoundaries,CreatedDate FROM Alerts where Expires >='".$newdate."' AND topic_id NOT IN (37,38,39,40) ORDER BY `Sent` DESC";   
     } else {
    $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,AffectedBoundaries,CreatedDate FROM Alerts where topic_id in (".$topicIDs.") AND  Expires >='".$newdate."' ORDER BY `Sent` DESC"; 
     }
    } else {
        $days_ago = date('Y-m-d H:i:s', strtotime('- '.$warning_days.' hours', strtotime($newdate)));
        if(in_array('all', $topic_type)) {
            $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,AffectedBoundaries,CreatedDate FROM Alerts where Expires >='".$days_ago."'  AND topic_id NOT IN (37,38,39,40)  order by `Sent` DESC"; 
        } else {
            $selectSQL = "SELECT id,AlertFullURL,topic_id,Subject,AffectedBoundaries,CreatedDate FROM Alerts where topic_id in (".$topicIDs.") AND Expires >='".$days_ago."'   order by `Sent` DESC"; 
        }
 }*/
...
if($record_type=='1') {
     if(in_array('all', $topic_type)) {
        $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate FROM Alerts where Expires >='".$newdate."' AND topic_id NOT IN (37,38,39,40) ORDER BY `Sent` DESC";   
     } else {
    $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate FROM Alerts where topic_id in (".$topicIDs.") AND  Expires >='".$newdate."' ORDER BY `Sent` DESC"; 
     }
    } else {
        $days_ago = date('Y-m-d H:i:s', strtotime('- '.$warning_days.' hours', strtotime($newdate)));
        if(in_array('all', $topic_type)) {
            $selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,CreatedDate FROM Alerts where Expires >='".$days_ago."'  AND topic_id NOT IN (37,38,39,40)  order by `Sent` DESC"; 
        } else {
            $selectSQL = "SELECT id,AlertFullURL,Subject,topic_id,CreatedDate FROM Alerts where topic_id in (".$topicIDs.") AND Expires >='".$days_ago."'   order by `Sent` DESC"; 
        }
 }
...
while($data=  mysql_fetch_array($result))
{
    ...
    if(in_array($data['topic_id'],$SevereWeather)) {
     ...
     $data['color']='#0000FF';
 } else if (($data['topic_id']=='36' || $data['topic_id']=='1') && (strpos($data['Subject'],'Bushfire Watch & Act') !== false) || (strpos($data['Subject'],'Bushfire Watch and Act') !== false) || (strpos($data['Subject'],'Bushfire Watch And Act') !== false) || (strpos($data['Subject'],'Bushfire Emergency Warning') !== false) || (strpos($data['Subject'],'Bushfire Advice') !== false))
{
    if ((strpos($data['Subject'],'Bushfire Watch & Act') !== false) || (strpos($data['Subject'],'Bushfire Watch And Act') !== false) || (strpos($data['Subject'],'Bushfire Watch and Act') !== false) || (strpos($data['Subject'],'Bushfire Emergency Warning') !== false) && ($detectByName=='Bushfire' || $detectByName=='Bushfire Watch')) 
    {
...
//die;
            ...
            //$data['color']='#FF0000';
                ...
                {  
                    ...
                    {
                        /* Remove bushfire warning from other type of warning */
