...
$Alert_array=array();
$selectSQL = "SELECT id,topic_id,AlertFullURL,Subject,AffectedBoundaries,Sent FROM Alerts where topic_id in (".$topicIDs.") AND  Sent >='".$todaydate."' AND Sent <='".$todaydateEnd."' AND (Expires < '".$tomorrow_dateEnd_time."' OR Expires >= '".$tomorrow_dateEnd_time."') AND is_deleted=0 ORDER BY `Sent` DESC";
