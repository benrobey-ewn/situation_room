/*
 *   This file seems to control the Maps Layer Data function
 *
 */
...
var kml_layer_marker_counter = 0;

var disableAllLayers = function () {
	client_checked_options.forEach(
		function(value) {
			clear_markers_layer_datas(value);
		}
	);
	multi_kml_marker_array.forEach(
		function(marker) {
			marker.setMap(null)
		}
	);
	$('#client_data_layer').prop('checked') ? $('#client_data_layer').click() : '';
	$('#public_data_layer').prop('checked') ? $('#public_data_layer').click() : '';
};

var generateSiteListFromKML = function(kmlurl, datalayerid) {
	var url = kmlurl.slice(6, kmlurl.length);
	var xml = $.get(url).done(function (result) {
		var html = '';
		$(result).find('Placemark').each(function(index) {
			var coords = $(this).find('Point').find('coordinates').html().slice(0, -2);
			var lat = coords.substr(0, coords.indexOf(','));
			var long = coords.substr(coords.indexOf(',')+1, coords.length);
			var tag = '<a style="cursor:pointer" onclick=setlocationnomarker(' + long + ',' + lat + ') >' + $(this).find('name').html() + '</a>';
			html += "<li>" + tag + "</li>";
		});

		$('#navigation_site_' + datalayerid).append(html);
	});
};
	...
	$('#select_client_data').change(function () {
			...
			$('#showClientDataDropDownDiv input:checked').each(function () {
				...
				if (e !== 'on') {
					if ((jQuery.inArray(e, current_selected_layer)) == -1) {
						if (e !== '12' && e !== '19' && e !== '38' && e !== '39' && e !== '45' && e !== '46' && e !== '47' && e !== '52' && e !== '53' && e !== '94' && e !== '95' && e !== '96' && e !== '104' ) {
		...
		onClick: function (view) {
			...
			if (view.checked == true) {
				...
				client_loaded_count = 0;
				if (view.value !== '12' && view.value !== '19' && view.value !== '38' && view.value !== '39' && view.value !== '45' && view.value !== '46' && view.value !== '47' && view.value !== '52' && view.value !== '53' && view.value !== '94' && view.value !== '95' && view.value !== '96' && view.value !== '104' ) {
					$('#site_list_option').show();
...
function load_layer(layer_values, layer_type) {
	...
	if(layer_values==100){
		...
		generateSiteListFromKML(CORE_NETWORK_SITES.url, layer_values);
	}
	if(layer_values==101){
		NFAS_SITES.setMap(map);
		generateSiteListFromKML(NFAS_SITES.url, layer_values);
	}
	if(layer_values==102){
		NWAS_SITES.setMap(map);
		generateSiteListFromKML(NWAS_SITES.url, layer_values);
	}

	if(layer_values==110){
		FTTN_SITES.setMap(map);
		generateSiteListFromKML(FTTN_SITES.url, layer_values);
	}
			...
			if (response.data.length > 0) {
				...
				for (k = 0; k < response.data.length; k++) {
					...
					if ((parseInt(response.data[k].layer_id) > 10 || parseInt(response.data[k].layer_id) == 2) && response.data[k].placemarker_name != "") {
						if(parseInt(response.data[k].layer_id) == 103 || response.data[k].layer_id == 105 | response.data[k].layer_id == 11) {
				...
				if ((parseInt(layer_values) > 10 || (parseInt(layer_values) == 2 && (LOGIN_USER_NAME == 'telstra' || LOGIN_USER_NAME == 'admin'))) && (parseInt(layer_values) !== 12 && parseInt(layer_values) !== 19 && parseInt(layer_values) !== 94 && parseInt(layer_values) !== 95 && parseInt(layer_values) !== 96 && parseInt(layer_values) !== 103 && parseInt(layer_values) !== 104 )) {
					//if statement above checks to see whether we should show the site list control
...
function getLoadedLayer(layer_id, layer_type) {
	...
	if(layer_id==102) {
		if (window.NWAS_SITES != null && window.NWAS_SITES != 'undefined') {
			var NWAS_SITES = window.NWAS_SITES;
			NWAS_SITES.setMap(map);
		}
		else {
			window.NWAS_SITES = new google.maps.KmlLayer('/kml_data/kml/NWAS_Sites.kml', {
				map: map,
				preserveViewport: true
			});
			window.NWAS_SITES.setMap(map);
		}
	}
	...
	if (client_layers.length > 0) {
		...
		for (z = 0; z <= client_layers.length - 1; z++) {
			...
			if ($('#showClientDataDropDownDiv').find("input[type=checkbox]").filter('[value="' + client_layers[z] + '"]').length > 0) {
				...
				// show sites
				if (client_layers[z] !== '12' && client_layers[z] !== '19' && client_layers[z] !== '38' && client_layers[z] !== '39' && client_layers[z] !== '45' && client_layers[z] !== '46' && client_layers[z] !== '47' && client_layers[z] !== '52' && client_layers[z] !== '53' && client_layers[z] !== '94' && client_layers[z] !== '95' && client_layers[z] !== '96' && client_layers[z] !== '104') {
					$('.assets_report').prop('disabled', false);
					$('.report_disabled').hide();
					$('#site_list_option').show();
