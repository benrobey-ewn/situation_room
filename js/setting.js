...
function settingsCheckOldCookie() {
	...
	if ($.cookie("SitRoomSettings") == null && oldSettingsCookie != null && oldSettingsCookie.length > 0) {
		...
		// DO NOT ADD TO THIS COOKIE MIGRATION Code
		// This migrates from the v1 to v2 cookie only.
...
function nbnDataLayerLoad(){
	if (settings.nbndataLayer !== null && typeof settings.nbndataLayer !== 'undefined' && settings.nbndataLayer.status!= false) {
		$('#checkbox-nbn-data-layers').attr("checked", true).trigger("change");
	}
	if (settings.drawPolygonSetting !== null && typeof settings.drawPolygonSetting !== 'undefined' && settings.drawPolygonSetting.status!= false) {
		$("#drawing_mode").attr("checked", true).trigger("change");
	}
}
...
function settingsSetSituationRoomObservations() {
	if (settings.observations !== null && typeof settings.observations !== 'undefined' && settings.observations.status != false) {
		...
		if (settings.observations.type == 3) {
			$('.observation_option3').attr("checked", true);
		}
...
function settingsSetSituationRoomRainfallGauges() {
	...
	//go through tick all boxes that need it.
...
function settingsSetSituationRoomRiverGauges() {
	if (settings.riverGauges !== null && typeof settings.riverGauges !== 'undefined' && settings.riverGauges.status != false) {
		...
		warning_option = settings.riverGauges.options;
		var $radios = $('input:checkbox[name="river_guage_options[]"]');
		$(warning_option).each(function (value) {
			var warning_value = warning_option[value];
			$radios.filter('[value="' + warning_value + '"]').prop('checked', true);
		});
		show_river_guages(settings.riverGauges.options);
		legendMainShow($('#river_gauges_codes_div'));
	}
	//go through tick all boxes that need it.
	else if (settings.riverGauges.options != null && typeof settings.riverGauges.options !== 'undefined') {
		warning_option = settings.riverGauges.options;
		var $radios = $('input:checkbox[name="river_guage_options[]"]');
		$(warning_option).each(function (value) {
			var warning_value = warning_option[value];
...
function settingsSetSituationRoomLayers() {
	...
	else if (settings.dataLayer !== true) {
		disableAllLayers();
	}
}
