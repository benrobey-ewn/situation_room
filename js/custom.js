...
$('.header-collapse').click(function (event) {
	...
	if ((event.target.nodeName != "LABEL" && event.target.nodeName != "INPUT")) {
		if (e.find('span').hasClass('glyphicon-chevron-down')) {  // Collapse.
			...
			$('html, #Popcontainer').animate({ scrollTop: $(e).offset().top }, 'slow');
		}
	}
});
	...
	var e = $(e);
		if (e.parent().hasClass('legend-container-content')) {
			...
			legends.each(function () {
				var f = $(this);
				if (f.parents().hasClass('legend-container-content')) {
					f.slideUp(400);
					f.parent().find('.rotate').removeClass('glyphicon-chevron-down').addClass('glyphicon-chevron-up');
				}
			});
		}
