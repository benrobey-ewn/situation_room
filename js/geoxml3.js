...
var render = function (responseXML, doc) {
    ...
    if (!!parserOptions.processStyles || !parserOptions.createMarker) {
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        if (node.getElementsByTagName('begin') && node.getElementsByTagName('begin').length > 0) {
          placemark.timeSpan.begin = geoXML3.nodeValue(node.getElementsByTagName('begin')[0]);
        }
        if (node.getElementsByTagName('end') && node.getElementsByTagName('end').length > 0) {
          placemark.timeSpan.end = geoXML3.nodeValue(node.getElementsByTagName('end')[0]);
        }
	/* JK
		if (node.attributes && (node.attributes.length > 0))
			placemark.id = node.attributes[0].value;
   */
   placemark.style = doc.styles[placemark.styleUrl] || clone(defaultStyle);
      ...
      for (i = 0; i < groundNodes.length; i++) {
        ...
        if (node.getElementsByTagName('begin') && node.getElementsByTagName('begin').length > 0)
        {
         groundOverlay.timeSpan.begin =  geoXML3.nodeValue(node.getElementsByTagName('begin')[0]);
       }
       if (node.getElementsByTagName('end') && node.getElementsByTagName('end').length > 0) {
         groundOverlay.timeSpan.end = geoXML3.nodeValue(node.getElementsByTagName('end')[0]);
       }
       if (!!window.google && !!google.maps) {
        doc.bounds = doc.bounds || new google.maps.LatLngBounds();
    ...
    if (!!window.google && !!google.maps) {
      ...
      if (!!style.href) {
        ...
        if (markerRegEx.test(style.href)) {
         //bottom middle
         var anchorPoint = new google.maps.Point(10*style.scale, 16*style.scale);
       } else {
        var anchorPoint = new google.maps.Point(10*style.scale, 10*style.scale);
      }
        // Init the style object with a standard KML icon
        style.icon =  new google.maps.MarkerImage(
          style.href,
          new google.maps.Size(10*style.scale, 10*style.scale),
          zeroPoint,
          // bottom middle 
          anchorPoint,
          new google.maps.Size(10*style.scale, 10*style.scale)
        ...
        var shadowSize = new google.maps.Size(59, 10);
        var shadowPoint = new google.maps.Point(8,10);
 ...
 */
 geoXML3.xmlParse = function (str) {
  if ((typeof ActiveXObject != 'undefined') || ("ActiveXObject" in window)) {
    /*var xmlDomObject = ['Msxml2.DOMDocument.6.0','Msxml2.XMLHTTP','Microsoft.XMLDOM'];
    for (var i = 0; i < xmlDomObject.length; i++) {
      try{
        var getAXO = xmlDomObject[i];
        var doc = new ActiveXObject(getAXO); break;
      } catch(e){
        continue;
      }
    }*/
