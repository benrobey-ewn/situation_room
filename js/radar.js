...
function setupMap(newmapElem) {
	...
	/* SEARCH BOX ON MAP CONTROLLERS */
	// Create the search box and link it to the UI element.
	var controllerInput = document.getElementById('pac-input');
	var controllerSearchBox = new google.maps.places.SearchBox(controllerInput);
	...
	poly.setMap(map);
	polygonPath.push(poly);
	poly.setPaths(new google.maps.MVCArray([polyPath]));

	google.maps.event.addListener(map, 'click', addPoint);
	/* New Polygon Creation */

	map.controls[google.maps.ControlPosition.TOP_CENTER].push(controllerInput);

	// Bias the SearchBox results towards current map's viewport.
	map.addListener('bounds_changed', function() {
		controllerSearchBox.setBounds(map.getBounds());
	});

	var locationMarkers = [];
  	// Listen for the event fired when the user selects a prediction and retrieve
  	// more details for that place.
 	controllerSearchBox.addListener('places_changed', function() {
    var places = controllerSearchBox.getPlaces();
...
//	if (useAjax) {
	...
	var url = "http://sitdata.ewn.com.au/radar/getRadarDev.php"; //Must be changed to getRadar.php for LIVE
	if (data == null || data.length == 0)
		data = '{}';
	$.ajax({
		type: "GET",
...
// temp work - start  ! delete this piece  when not required
var $radarPolling;
var $radarPollingInterval = 180000; //300000;//60000;  


function radarAutoRefresh(){
	console.log("radar auto refresh started");
	 $radarPolling = setInterval(function(){
	 	staticRadarTimes.length = 0;
	 	 getRadarsViewPortChanged();
	 },$radarPollingInterval);
}

function clearAutoRefreshInterval(){
	
	if(typeof $radarPolling !== undefined){
		clearInterval($radarPolling);
		console.log("radar auto refresh stopped");
	}
}
// temp work - end  ! delete this piece when not required
...
//	$('#optRadar').prop('checked', true);
	...
	ChangeRadarOpacity();
	// auto refresh radar Images after a interval
	radarAutoRefresh();
}
...
function initEWNMapsRadarPrototype() {
	...
	EWNMapsRadar.prototype.showNextImage = function (matchTime) {
		try {
			if (this.nextImage() == undefined || this.img_ == undefined && this.images_ > 0) {
				// maps aren't fully loaded yet
				staticRadarTimesX = -1;
				setRadarImagesTimer(1000);
			}
			if (this.images_.length == 0 || this.nextImage() == undefined)
				return;
			// radar time sync
	...
	EWNMapsRadar.prototype.showPrevImage = function (matchTime) {
		try {
			if (this.prevImage() == undefined || this.img_ == undefined && this.images_ > 0) {
				// maps aren't fully loaded yet
				staticRadarTimesX = -1;
				setRadarImagesTimer(1000);
			}
